<template>
  <!-- 修改部门 -->
  <div class="reuse">
    <div v-if="$store.state.home.open" class="title">修改部门</div>
    <div v-else class="title">update Department</div>
    <div class="form_box">
      <form action>
...
export default {
  ...
  methods: {
    ...
    updateAction() {
      ...
      request.post("/creaDepartment/update", data).then(res => {
  ...
  computed: {
    list_checked() {
      ...
      var two = [];
      var item = [];
      if (_this.mentList.assessId) {
        let one = _this.mentList.assessId;
        // console.log(one);
        two = one.split(",");
        two.map(data => {
